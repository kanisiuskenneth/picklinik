...
class PrescriptionInProgress extends React.Component {

    state = {
       title: "Resep Anda Sedang Diproses",
        circularProgressMode: "indeterminate",
        circularValue: 100,
        buttonLabel: "Batalkan Pesanan",
        done: {visibility: 'hidden'},
        iconClass: 'flaticon-prescription',
    };

    handleButton = () => {
      this.props.history.push("/farmasi")
    };

    componentDidMount() {
        sleep(8000).then(() => {
            this.setState({title: "Resep anda telah diterima", done: true, buttonLabel:"Kembali", iconClass: 'flaticon-checked'})
        })
    }
...
export default PrescriptionInProgress;